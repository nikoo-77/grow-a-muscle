...
export default function CommunityPage() {
  ...
  const [comments, setComments] = useState<{ [key: string]: any[] }>({}); // { postId: [comments] }
  const [likes, setLikes] = useState<{ [key: string]: any[] }>({}); // { postId: [likes] }
  const [liking, setLiking] = useState<{ [key: string]: boolean }>({}); // { postId: boolean }
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'users' }, payload => {
        // When any user profile changes, re-fetch all user profiles
        const allUserIds = new Set<string>();
        posts.forEach(post => allUserIds.add(post.user_id));
        Object.values(comments).flat().forEach(comment => allUserIds.add(comment.user_id));
        if (allUserIds.size > 0) {
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto py-16 px-4 bg-white mt-32">
        ...
        <p className="text-lg text-center mb-8 text-[#60ab66]">Share your progress, ask questions, and connect with others!</p>
        {/* Post creation box */}
        <div className="bg-white rounded-3xl shadow-2xl p-6 mb-8 flex flex-col gap-4">
          <div className="flex items-start gap-4">
            <img src={currentUserProfile?.profile_picture || '/logo1.png'} alt="Your avatar" className="w-14 h-14 rounded-full border-2 border-[#60ab66] object-cover" />
            <textarea
        ...
        <div className="flex flex-col gap-6">
          {posts.length === 0 ? (
            ...
            posts.map(post => {
              ...
              return (
                <div key={post.id} className="bg-white rounded-3xl shadow-2xl p-6 flex flex-col gap-4 relative group hover:shadow-3xl transition-all duration-300">
                  <div className="flex gap-4 items-start">
                    <img src={userProfiles[post.user_id]?.profile_picture || '/logo1.png'} alt="User avatar" className="w-14 h-14 rounded-full border-2 border-[#60ab66] object-cover" />
                    <div className="flex-1 flex flex-col">
                      ...
                      {post.image_url && (
                        <img src={post.image_url} alt="Post image" className="rounded-xl max-h-80 object-contain border border-[#e0e5dc] mb-3" />
                      )}
                      <div className="flex gap-6 mt-auto">
                        <button
                          className={`flex items-center gap-1 font-semibold transition ${userLiked ? 'text-[#2e3d27]' : 'text-[#60ab66] hover:text-[#2e3d27]'}`}
                          onClick={() => handleLike(post.id)}
                          disabled={liking[post.id]}
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill={userLiked ? '#60ab66' : 'none'} viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 9l-2-2-2 2m0 6l2 2 2-2" /></svg>
                          {postLikes.length}
                        </button>
                        <button
                        ...
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8s-9-3.582-9-8a9 9 0 1118 0z" /></svg>
                  ...
                  <div className="mt-2">
                    <div className="flex flex-col gap-2">
                      {(comments[post.id] || []).map(comment => (
                        <div key={comment.id} className="flex gap-2 items-start bg-[#f6f9f6] rounded-xl px-4 py-2">
                          <img src={userProfiles[comment.user_id]?.profile_picture || '/logo1.png'} alt="User avatar" className="w-8 h-8 rounded-full border border-[#60ab66] object-cover" />
                          <div>
                    ...
                    {user && (
                      <div className="flex gap-2 mt-2 items-start">
                        <img src={userProfiles[user.id]?.profile_picture || '/logo1.png'} alt="User avatar" className="w-8 h-8 rounded-full border border-[#60ab66] object-cover" />
                        <textarea
